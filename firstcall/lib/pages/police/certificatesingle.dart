...
class CertificateSingle extends StatefulWidget {
  const CertificateSingle({super.key, this.id});
  final String? id;

  @override
  State<CertificateSingle> createState() => _CertificateSingleState();
}

class _CertificateSingleState extends State<CertificateSingle> {
  ...
  updateStatus(status) async {
    ...
    try {
      ...
      final response = await _certificateService.updateCertificateStatus(data);
      print(response.data);
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      children: [
                        ListTile(
                          title: Text(certificate['userid']['name']),
                          subtitle:
                              Text(certificate['userid']['phone'].toString()),
                        ),
                        const SizedBox(
                          height: 10,
                        ),
                        ...
                        Row(
                          children: [
                            ElevatedButton(
                                onPressed: () {
                                  updateStatus("Under Verification");
                                },
                                child: Text("Under Verification")),
                            ElevatedButton(
                                onPressed: () {
